...
async function markItem(configs, item, as) {
    if (item.serviceRef) {
        try {
...
exports.feverServiceHooks = {
    ...
    updateSources: () => async (dispatch, getState) => {
        ...
        const feedGroups = response.feeds_groups;
        if (feeds === undefined)
            throw APIError();
        let groupsMap;
        if (configs.importGroups) {
            // Import groups on the first sync
            const groups = (await fetchAPI(configs, "&groups")).groups;
        ...
        const sources = feeds.map(f => {
            const source = new source_1.RSSSource(f.url, f.title);
        ...
        if (items.length > 0) {
            ...
            const parsedItems = items.map(i => {
                ...
                const item = {
                    ...
                    content: i.html,
                    snippet: (0, utils_1.htmlDecode)(i.html).trim(),
                    creator: i.author,
                ...
                if (img && img.src) {
                    ...
                    let a = dom.querySelector("body>ul>li:first-child>a");
    ...
    syncItems: () => async (_, getState) => {
        ...
        const unreadFids = unreadResponse.unread_item_ids.split(",");
        const starredFids = starredResponse.saved_item_ids.split(",");
    ...
    markRead: (item) => async (_, getState) => {
        await markItem(getState().service, item, "read");
    },
    markUnread: (item) => async (_, getState) => {
        await markItem(getState().service, item, "unread");
    },
    star: (item) => async (_, getState) => {
        await markItem(getState().service, item, "saved");
    },
    unstar: (item) => async (_, getState) => {
        await markItem(getState().service, item, "unsaved");
    },
};
